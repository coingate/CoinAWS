...
func main() {
	...
	cfgAWS, err := aws.GetConfigWithProfile(profile)
	if err != nil {
	...
	for {
		// Select AWS secret
		secretName, err := aws.SelectSecret(cfgAWS)
		if err != nil {
		...
		// Edit AWS secret
		originalSecret, editedSecret, err := aws.EditSecret(cfgAWS, secretName, cfg.DefaultEditor)
		if err != nil {
		...
		if versionLabel == "" {
			versionLabel = aws.GenerateVersionLabel()
		}
		...
		// Update AWS secret
		err = aws.UpdateSecret(cfgAWS, secretName, editedSecret, versionStages)
		if err != nil {
